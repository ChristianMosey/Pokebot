...
// src/extractors/YoutubeExtractor.ts
var YouTubeLibs = ["play-dl", "ytdl-core"];
...
var _YoutubeExtractor = class extends import_discord_player3.BaseExtractor {
  async activate() {
    let lib, isYtdl = false;
    for (const ytlib of YouTubeLibs) {
      if (lib = import_discord_player3.Util.require(ytlib)) {
    ...
    this._spotify = (0, import_spotify_url_info.default)(import_node_fetch2.default);
  }
  async validate(query, type) {
    if (typeof query !== "string")
      return false;
  ...
  async handle(query, context) {
    ...
    switch (context.type) {
      ...
      case import_discord_player3.QueryType.YOUTUBE_VIDEO: {
        ...
        if (!id?.[0])
          return this.emptyResponse();
        const video = await import_youtube_sr.YouTube.getVideo(`https://www.youtube.com/watch?v=${id}`).catch(import_discord_player3.Util.noop);
        if (!video)
          return this.emptyResponse();
      ...
      case import_discord_player3.QueryType.SPOTIFY_SONG: {
        const spotifyData = await this._spotify.getData(query).catch(import_discord_player3.Util.noop);
        if (!spotifyData)
          return { playlist: null, tracks: [] };
      ...
      case import_discord_player3.QueryType.SPOTIFY_PLAYLIST: {
        const spotifyPlaylist = await this._spotify.getData(query).catch(import_discord_player3.Util.noop);
        if (!spotifyPlaylist)
          return { playlist: null, tracks: [] };
        const playlist = new import_discord_player3.Playlist(this.context.player, {
      ...
      case import_discord_player3.QueryType.SPOTIFY_ALBUM: {
        const album = await this._spotify.getData(query).catch(import_discord_player3.Util.noop);
        if (!album)
          return { playlist: null, tracks: [] };
        const playlist = new import_discord_player3.Playlist(this.context.player, {
  ...
  async handle(query, context) {
    switch (context.type) {
      case import_discord_player6.QueryType.ARBITRARY: {
        const data = await downloadStream(query);
