  ...
  async handle(query, context) {
    ...
    switch (context.type) {
      ...
      case QueryType2.YOUTUBE_VIDEO: {
        ...
        if (!id?.[0])
          return this.emptyResponse();
        const video = await YouTube.getVideo(`https://www.youtube.com/watch?v=${id}`).catch(Util2.noop);
        if (!video)
          return this.emptyResponse();
      ...
      case QueryType2.SPOTIFY_SONG: {
        const spotifyData = await this._spotify.getData(query).catch(Util2.noop);
        if (!spotifyData)
          return { playlist: null, tracks: [] };
      ...
      case QueryType2.SPOTIFY_PLAYLIST: {
        const spotifyPlaylist = await this._spotify.getData(query).catch(Util2.noop);
        if (!spotifyPlaylist)
          return { playlist: null, tracks: [] };
        const playlist = new Playlist2(this.context.player, {
      ...
      case QueryType2.SPOTIFY_ALBUM: {
        const album = await this._spotify.getData(query).catch(Util2.noop);
        if (!album)
          return { playlist: null, tracks: [] };
        const playlist = new Playlist2(this.context.player, {
